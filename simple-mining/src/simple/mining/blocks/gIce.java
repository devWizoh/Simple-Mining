        ...
        if (!event.isCancelled()) {
            ...
            if (player.getWorld().getName().equalsIgnoreCase("Minas") && bloco.getType() == Material.PACKED_ICE) {
                ...
                if (player.hasPermission("bonus.magic")) {
                    Core.economy.depositPlayer((OfflinePlayer) player, 100D); /* com multiplicador */
                    ActionBar.sendActionBarMessage(player, "§e§lMINERAÇÃO §7§l▏ §fForam adicionados §a+$100 §fem sua conta. §7[Bônus de 2.5x]");
                }
            }
        }
    }
